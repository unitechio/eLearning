...
export const useAdminBillingInvoices = (query: adminService.AdminUserQuery = { page: 1, page_size: 50 }) =>
  useQuery({
    queryKey: ['admin', 'billing', 'invoices', query],
    queryFn: () => adminService.listAdminBillingInvoices(query),
  });

export const useAdminPaymentTransactions = (query: adminService.AdminUserQuery = { page: 1, page_size: 50 }) =>
  useQuery({
    queryKey: ['admin', 'billing', 'payments', query],
    queryFn: () => adminService.listAdminPaymentTransactions(query),
  });
