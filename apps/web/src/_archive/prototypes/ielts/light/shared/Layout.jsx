  ...
  return (
    <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-50">
      <div className="flex items-center gap-2">
        ...
        <span className="font-black text-gray-900 text-sm">UNI TỰ HỌC</span>
      </div>
    ...
    <footer className="bg-gray-900 text-gray-400 px-8 py-10 mt-16">
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
        <div>
          ...
          <p className="text-xs leading-relaxed">Một sản phẩm thuộc Học viện Tiếng Anh Tự Duy UNI English (IELTS Đình Lực)</p>
        </div>
        {[
          { title: "LUYỆN THI IELTS", items: ["IELTS Online Test", "IELTS Reading Practice", "IELTS Listening Practice"] },
          { title: "VỀ UNI IELTS ĐÌNH LỰC", items: ["Linearthinking", "Nền tảng công nghệ", "Đội ngũ giáo viên"] },
          { title: "UNI ECOSYSTEM", items: ["UNI Grammar", "UNI Dictionary", "UNI superLMS"] },
        ].map((col) => (
          <div key={col.title}>
      ...
      <div className="max-w-6xl mx-auto mt-6 pt-6 border-t border-gray-800 text-xs text-center text-gray-600">
        © 2024 UNI English. All rights reserved.
      </div>
    </footer>
  );
}
  ...
  return (
    <div className="bg-red-50 border border-red-100 rounded-2xl p-8 flex items-center justify-between mx-4 md:mx-0 mt-12">
      <div>
        ...
        <button className="mt-3 bg-red-600 hover:bg-red-700 text-white text-sm font-bold px-5 py-2 rounded-[8px] transition-colors">Gia hạn miễn phí</button>
      </div>
