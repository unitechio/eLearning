// ============================================================
// UNI / SENGLISH — Full App (all pages, components, layouts)
        ...
        {NAV.map((group) => (
          <div key={group.section}>
            ...
            {group.items.map((item) => (
              <button
                key={item.id}
                className={`w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm transition-colors ${item.id === active
...
// ── LAYOUT: UniHeader ───────────────────────────────────────────────────────
// components/layout/UniHeader.jsx
// Used by: Writing, IELTS Test, Dictation pages (UNI Tự Học domain)

const NAV_ITEMS = ["IELTS Online Test ▾", "Bài mẫu IELTS ▾", "Chép chính tả"];

function UniHeader({ active = "" }) {
  return (
    <header className="bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between sticky top-0 z-50">
      ...
      <nav className="hidden md:flex items-center gap-6">
        {NAV_ITEMS.map((item) => (
          <button
            key={item}
            className={`text-sm font-medium transition-colors hover:text-red-600 ${active === item ? "text-red-600" : "text-gray-600"
              }`}
          >
            {item}
          </button>
...
// ── LAYOUT: UniFooter ───────────────────────────────────────────────────────
// components/layout/UniFooter.jsx
// Used by all public UNI Tự Học pages (not full-screen test/practice pages)
...
// Reusable hero section for UNI Tự Học public pages
...
function RedButton({ children, onClick, className = "", small = false }) {
  return (
    <button
      onClick={onClick}
      className={`bg-red-600 hover:bg-red-700 text-white font-bold transition-colors rounded-full ${small ? "text-xs px-4 py-1.5" : "text-sm px-6 py-2.5"
        } ${className}`}
    >
      {children}
    </button>
    ...
    <div className="flex items-center gap-1 mt-8">
      {Array.from({ length: total }, (_, i) => i + 1).map((p) => (
        <button
          key={p}
          onClick={() => onChange?.(p)}
          className={`w-8 h-8 rounded-full text-sm font-semibold transition-all ${p === current
...
// Reusable audio player strip — works in both light (UNI) and dark (Senglish) themes.
//
// Props:
...
function AudioPlayer({
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="flex gap-0.5 shrink-0">
        {SPEEDS.map((s) => (
          <button
            key={s}
            onClick={() => onSpeed?.(s)}
            className={`text-xs px-1.5 py-0.5 rounded transition-all ${speed === s ? pillOn : pill
              }`}
          >
            {s}
          </button>
      ...
      {/* Loop */}
      <button className={`text-xs border rounded px-2 py-1 transition-colors shrink-0 ${dark ? "border-gray-700 text-gray-500 hover:border-gray-500" : "border-gray-300 text-gray-500 hover:border-red-400"
        }`}>
        ⟳
      </button>
    </div>
      ...
      {Array.from({ length: total }, (_, i) => i + 1).map((n) => {
        ...
        return (
          <button
            key={n}
            onClick={() => onSelect?.(n)}
            className={`w-8 h-8 rounded-full text-xs font-medium transition-all ${isActive
    ...
    <div className={`flex gap-1 ${dark ? "bg-gray-800" : "bg-gray-100"} p-1 rounded-xl`}>
      {tabs.map((tab) => (
        <button
          ...
          className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-semibold transition-all ${active === tab
            ? "bg-red-600 text-white shadow-sm"
            : dark
...
function LessonCard({ lesson, dark = false }) {
  ...
  return (
    <div className={`rounded-xl p-4 cursor-pointer group transition-all ${dark
      ...
      <p className={`text-sm font-semibold leading-snug line-clamp-2 group-hover:text-red-500 transition-colors ${dark ? "text-gray-100" : "text-gray-900"
        }`}>
        {lesson.title}
      </p>
      <div className="mt-3">
          ...
          <div className="mt-4 inline-flex gap-1 bg-gray-800 p-1 rounded-full">
            {["Tuần", "Tháng"].map((p) => (
              <button
                key={p}
                onClick={() => setPeriod(p)}
                className={`px-5 py-1.5 rounded-full text-sm font-semibold transition-all ${period === p ? "bg-red-600 text-white" : "text-gray-400 hover:text-white"
                  }`}
              >
                {p}
              </button>
          ...
          <div className="mt-3 flex justify-center gap-2">
            {[
              ...
              <button
                key={key}
                onClick={() => setTab(key)}
                className={`flex items-center gap-1 px-4 py-1.5 rounded-full text-xs font-medium border transition-all ${tab === key
            ...
            <div className="flex gap-2 flex-wrap">
              {["Dictation", "Shadowing", "Luyện nói", "Từ vựng"].map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${tab === t
                    ? "bg-red-600 text-white"
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col items-center gap-3">
        <button
          ...
          className={`w-20 h-20 rounded-full flex items-center justify-center text-3xl transition-all shadow-lg ${state === "recording"
              ...
              <div className="w-44 shrink-0 space-y-2">
                {IELTS_PARTS.map((p, i) => (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-xl transition-all ${partIdx === i
                      ? "bg-red-600 text-white"
                ...
                <div className="flex items-center gap-2">
                  {part.questions.map((_, i) => (
                    <button
                      key={i}
                      onClick={() => setQIdx(i)}
                      className={`w-7 h-7 rounded-full text-xs font-semibold transition-all ${qIdx === i
                        ? "bg-red-600 text-white"
              ...
              <div className="flex gap-1 border-b border-gray-800">
                {["Định nghĩa", "Ví dụ", "Từ liên quan"].map((t) => (
                  <button
                    key={t}
                    onClick={() => setTab(t)}
                    className={`px-4 py-2 text-sm font-semibold transition-all border-b-2 -mb-px ${tab === t
      ...
      <div className="flex gap-1">
        {SPEEDS.map((s) => (
          <button
            key={s}
            onClick={() => onSpeed(s)}
            className={`text-xs px-2 py-0.5 rounded transition-all ${speed === s
...
function SentenceItem({ s, isCurrent, onClick }) {
  return (
    <div
      onClick={onClick}
      className={`rounded-xl p-3 cursor-pointer transition-all ${isCurrent
    ...
    >
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          <div
            className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition-all ${isCurrent ? "border-red-600 bg-red-600" : "border-gray-600"
              }`}
          >
      ...
      <div className="px-4 py-2 flex items-center justify-between text-xs text-gray-500">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`w-8 h-4 rounded-full transition-colors relative ${showSubs ? "bg-blue-600" : "bg-gray-700"
              }`}
          >
            <div className={`w-3 h-3 bg-white rounded-full absolute top-0.5 transition-all ${showSubs ? "left-4" : "left-0.5"
              }`} />
          </button>
        </div>
      </div>
    ...
    <div className="flex min-h-screen bg-gray-950 text-white font-sans">
      ...
      <main className="flex-1 p-8 flex flex-col gap-5 overflow-y-auto">
        ...
        <div className="bg-gray-900 rounded-2xl p-6 border border-gray-800">
          ...
          >
            <div
              className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-all ${recording
...
function DictationListPage() {
  return (
    <div className="min-h-screen bg-white font-sans">
      <UniHeader active="Chép chính tả" />

      <HeroBanner
        title="Chép Chính Tả Tiếng Anh -"
        titleRed="UNI Tự học"
        description="Website nghe chép chính tả Tiếng Anh UNI Tự học cung cấp nguồn nghe chép chính tả đa dạng theo chủ đề và độ khó của bài nghe, kèm transcript tiếng anh và tiếng việt."
      />
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1 mt-8">
            {[1, 2, 3, 4, 5].map((p) => (
              <button
                key={p}
                className={`w-8 h-8 rounded-full text-sm font-semibold transition-all ${p === 1
                  ? "bg-red-600 text-white"
                  : "border border-gray-300 text-gray-600 hover:border-red-400"
                  }`}
              >
      ...
      <div className="grid grid-cols-5 gap-1.5">
        {Array.from({ length: TOTAL }, (_, i) => i + 1).map((n) => (
          <button
            key={n}
            onClick={() => onSelect(n)}
            className={`w-8 h-8 rounded-full text-xs font-medium transition-all ${n === current
              ? "bg-red-600 text-white shadow-sm"
              : n < current
      ...
      <div className="flex items-center gap-1">
        {speeds.map((s) => (
          <button
            ...
            className={`text-xs px-1.5 py-0.5 rounded transition-all ${speed === s
              ? "bg-gray-800 text-white"
                ...
                <div className="flex items-center gap-2">
                  {["EASY", "HARD"].map((m) => (
                    <button
                      key={m}
                      onClick={() => setMode(m)}
                      className={`text-xs font-bold px-3 py-1 rounded border transition-all ${mode === m
              ...
              <div className="px-5 py-4">
                ...
                <div className="flex items-start justify-between mt-3 gap-2">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    <button
                      disabled={!input.trim()}
                      className={`text-sm px-4 py-1.5 rounded-lg font-semibold transition-all ${input.trim()
...
// pages/UNI/IELTSTestListPage.jsx
...
function BookCard({ b }) {
  return (
    <div className="cursor-pointer group">
      <div
        className={`${b.color} rounded-xl h-32 flex items-center justify-center text-4xl ${b.dark ? "text-white" : ""
          }`}
      >
        {b.emoji}
      </div>
...
function IELTSTestListPage() {
  return (
    <div className="min-h-screen bg-white font-sans">
      <UniHeader active="IELTS Online Test ▾" />

      <HeroBanner
        title="Luyện thi IELTS Online Test"
        titleRed="miễn phí - UNI Tự học"
        description="Luyện thi IELTS Online Test miễn phí cùng UNI Tự học. Trải nghiệm thi thật với giao diện trực tuyến kèm giải thích chi tiết với Linearthinking."
      />
...
const PAGES = [
  ...
  { id: "ielts-test-list", label: "📝 Test List", domain: "UNI", Component: IELTSTestListPage },
  { id: "writing-list", label: "📋 Writing List", domain: "UNI", Component: WritingListPage },
  { id: "writing-detail", label: "✍️ Writing Detail", domain: "UNI", Component: WritingDetailPage },
  { id: "dictation-list", label: "🎙 Dictation List", domain: "UNI", Component: DictationListPage },
  { id: "dictation-practice", label: "✏️ Dictation Practice", domain: "UNI", Component: DictationPracticePage },
];

const DOMAINS = ["Senglish", "IELTS Test", "UNI"];
            ...
            <div key={domain} className="flex items-center gap-0.5 mr-4 shrink-0">
              ...
              {PAGES.filter((p) => p.domain === domain).map((p) => (
                <button
                  ...
                  className={`whitespace-nowrap px-2.5 py-1 rounded text-xs font-medium transition-all ${active === p.id
                    ? "bg-red-600 text-white"
