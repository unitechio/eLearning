...
type WebSocketService interface {
	...
	RegisterClient(client *model.WebSocketClient)

	// UnregisterClient unregisters a WebSocket client
	UnregisterClient(clientID string)

	// SendToUser sends a message to a specific user (all their connections)
	SendToUser(userID uuid.UUID, message *model.WebSocketMessage) error

	// SendToClient sends a message to a specific client connection
	SendToClient(clientID string, message *model.WebSocketMessage) error

	// Broadcast sends a message to all connected clients
	Broadcast(message *model.WebSocketMessage) error

	// BroadcastExcept sends a message to all clients except the specified one
	BroadcastExcept(excludeClientID string, message *model.WebSocketMessage) error
	...
	NotifyNotification(notification *model.Notification, action string) error
}
