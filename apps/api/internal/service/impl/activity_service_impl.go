...
func (s *ActivityService) GetActivity(id string) (*dto.Activity, error) {
...
func (s *ActivityService) CreateActivity(req dto.UpsertActivityRequest) (*dto.Activity, error) {
...
func (s *ActivityService) UpdateActivity(id string, req dto.UpsertActivityRequest) (*dto.Activity, error) {
	activityID, err := uuid.Parse(id)
	if err != nil {
		return nil, apperr.BadRequest("invalid activity id")
	}
...
func (s *ActivityService) SubmitActivity(id string, userID uuid.UUID, req dto.SubmitActivityRequest) (*dto.Submission, error) {
	activityID, err := uuid.Parse(id)
	if err != nil {
		return nil, apperr.BadRequest("invalid activity id")
	}
...
func (s *ActivityService) ListActivitySubmissions(id string, query dto.ActivitySubmissionListQuery) (*dto.PageResult[dto.Submission], error) {
	activityID, err := uuid.Parse(id)
	if err != nil {
		return nil, apperr.BadRequest("invalid activity id")
	}
...
func (s *ActivityService) GetSubmission(id string) (*dto.Submission, error) {
...
func mapActivity(item *model.Activity) *dto.Activity {
...
func mapSubmission(item *model.ActivitySubmission) *dto.Submission {
	score := 0.0
	if item.Score != nil {
		score = *item.Score
	}
