...
type WritingExtrasService struct {
	repo repository.WritingRepository
	llm  ai.LLMService
}

func NewWritingExtrasService(repo repository.WritingRepository, llm ai.LLMService) *WritingExtrasService {
	return &WritingExtrasService{repo: repo, llm: llm}
}

func (s *WritingExtrasService) GetWritingByID(userID uuid.UUID, id string) (map[string]any, error) {
...
func (s *WritingExtrasService) EvaluateWriting(req dto.WritingEvaluationRequest) (map[string]any, error) {
